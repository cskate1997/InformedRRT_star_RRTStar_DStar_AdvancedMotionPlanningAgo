...
class DStar:
    ...
    def process_state(self):
        ...
        if k_old < x.h:
            for n in self.get_neighbors(x):
                if n.h <= k_old and x.h > (n.h + self.cost(n,x)):
                    x.parent = n
                    x.h = n.h + self.cost(n,x)
        if k_old == x.h:
            for n in self.get_neighbors(x):
                if n.tag == "NEW" or (n.parent == x and n.h != x.h + self.cost(n,x)) or (n.parent != x and n.h > x.h + self.cost(n,x)):
                    n.parent = x
                    self.insert(n,(x.h + self.cost(n,x)))
        else:
            for n in self.get_neighbors(x):
                if n.tag == "NEW" or (n.parent == x and n.h != x.h + self.cost(n,x)):
                    n.parent = x
                    self.insert(n, (x.h + self.cost(n,x)))
                else:
                    if n.parent != x and n.h > x.h + self.cost(n,x):
                        self.insert(n, x.h)
                    else:
                        if n.parent != x and x.h > n.h + self.cost(n,x) and n.tag == "CLOSED" and n.h > k_old:
                            self.insert(n, n.h)
    ...
    def repair_replan(self, node):
        ...
        while k_min < n.h and k_min != -1:
            k_min = self.process_state()
            # p = self.get_backpointer_list(n)
    ...
    def prepare_repair(self, node):
        ...
        s_neighbors = self.get_neighbors(node)
        for n in s_neighbors:
            if n.is_dy_obs and not n.is_obs:
                n.is_obs = True
                n_neigh = self.get_neighbors(n)
                for nn in n_neigh:
