...
class RRT:
    ...
    def get_new_point_in_ellipsoid(self, goal_bias, c_best):
        ...
        # Generate a random point in an ellipsoid
            ...
            x_center = np.atleast_2d([(self.start.row+self.goal.row)/2,(self.start.col+self.goal.col)/2,0])
            # print("KKK",x_center.shape)
            # Compute rotation matrix from elipse to world frame - C
            aa = self.start.row
            ab = self.goal.row
            ba = self.start.col
            bb = self.goal.col
            a1 = np.append(np.array((np.array([aa,ba])-np.array([ab,bb]))/(c_min)),0)
            a1=np.atleast_2d(a1)
            # print(a1)
            #a1 = np.array((np.array(self.start)- np.array(self.goal))/(c_min))
            W1 = [1]+[0]*(d - 1)
            # print(W1)
            M = np.outer(a1,W1)
            # print(M)
            ...
            u = u/norm
            x_ball = np.atleast_2d(u[0,:dim])
            # print("x_ball", x_ball.shape)
            # Map ball sample to the ellipsoid - x_rand
            x_rand = (C@L@x_ball.T).T + x_center
            # print(x_rand)
